...
#        self.set_led(8, 0)
    ...
    def fastSensingOverSignal(self):
        ...
        if res == vrep.simx_return_ok:
            ...
            self._proximitySensorValues = np.array(sensValues[:8], dtype=float)
            self._lightSensorValues = np.array(sensValues[8:16], dtype=float)
            self._groundSensorValues = np.array(sensValues[16:19], dtype=float)
            self._accelerometerValues = np.array(sensValues[19:22], dtype=float)
            self._wheelEncoderValues = np.array(sensValues[22:], dtype=float)
